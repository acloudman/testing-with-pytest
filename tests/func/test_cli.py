...
def test_list_no_args(mocker):
    mocker.patch.object(cli, '_tasks_db', new=stub_tasks_db)
    mocker.patch.object(tasks, 'list_tasks', return_value=[])
    runner = CliRunner()
    runner.invoke(cli.tasks_cli, ['list'])
    cli.tasks.list_tasks.assert_called_once_with(None)
...
def test_list_print_many_items(no_db, mocker):
    ...
    mocker.patch.object(tasks, 'list_tasks',
...
def test_list_dash_o(no_db, mocker):
    mocker.patch.object(tasks, 'list_tasks')
    runner = CliRunner()
    runner.invoke(cli.tasks_cli, ['list', '-o', 'brian'])
    tasks.list_tasks.assert_called_once_with('brian')


def test_list_dash_dash_owner(no_db, mocker):
    mocker.patch.object(tasks, 'list_tasks')
    runner = CliRunner()
    runner.invoke(cli.tasks_cli, ['list', '--owner', 'okken'])
    tasks.list_tasks.assert_called_once_with('okken')